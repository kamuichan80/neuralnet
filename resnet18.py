...
class ResNet18(nn.Module):
    ...
    def __init__(self, block: Type[Union[BasicBlock, Bottleneck]], layers: List[int], num_classes: int = 1000, zero_init_residual: bool = False, groups: int = 1, width_per_group: int = 64):
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, dilate=self.dilation)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, dilate=self.dilation)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, dilate=self.dilation)
